...
jQuery(function () {
    jQuery().delay( 1000 ); // Nasty :(
    jQuery('iframe').load(function () {
        attachCssToPreviewIframe();
        console.log('laod the iframe')
    });
    attachCssToPreviewIframe();
});
    function attachCssToPreviewIframe() {
        // var $head = jQuery('iframe').contents().find("head");

       // $head.append($("<link/>",
       //     { rel: "stylesheet", href: window.AppRequestContextPath+"/app/resources/styles/page/article-editor-iframe.css", type: "text/css" }
       // ));
    }